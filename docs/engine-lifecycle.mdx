...
An **externally managed** engine meets those requirements without PAIR owning
it: PAIR observes the server, lists its models, and routes to it, while the
server's processes, models, and installation stay under whatever already runs
them. Everything in this page about installing, starting, stopping, updating,
uninstalling, and downloading or unloading models applies only to the engines
PAIR manages. See [Using an existing llama-swap server](llama-swap.mdx) for the
...
- [Using an existing llama-swap server](llama-swap.mdx)
