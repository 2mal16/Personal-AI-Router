...
An engine labelled `(external)` is managed outside PAIR — see
[Using an existing llama-swap server](llama-swap.mdx). None of the keys above act
on it: each one reports that its processes and models belong to its own
application, and quitting the terminal interface leaves it running. Change the
port PAIR connects to from the desktop application or a manifest override, not
from here.
...
switch between the two proxies — Ollama, and the OpenAI-compatible one shared by
LM Studio and llama-swap — `enter` to pin the highlighted upstream, and `a`
...
- [Using an existing llama-swap server](llama-swap.mdx) — the externally managed
  engine and what the terminal interface will not do to it
- [Ports](getting-started.mdx#7-connecting-your-agents-and-port-information) —
